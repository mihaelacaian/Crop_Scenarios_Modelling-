...
####### 1. compute efficiency: ENS8_prep_Heff_Fxx_rev.dat 
...
* create  the ENS8_prep_Heff_F*_rev.dat file containing (Harvest(date)-Harvest(date1))/Harvest(date1) for each scenario
***************************************************************************************************************************
suf1='prep_Heff'
suf2='rev.ctl'
...
'set gxout fwrite'
'set fwrite ENS8_CH_DATE_rev3.dat'
...
while(it<=4)
'set t 'it
'define dh=(eh.1-h0)/h0(t=1)'
'define dr=(er.1-r0)/r0(t=1)'
'define drr=(err.1-rr0)/rr0(t=1)'
'd dh'
'd dr'
'd drr'
it=it+1
...
while(it<=4)
'set t 'it
'define dh1=(eh.2-h0)/h0(t=1)'
'define dr1=(er.2-r0)/r0(t=1)'
'define drr1=(err.2-rr0)/rr0(t=1)'
'd dh1'
'd dr1'
'd drr1'
it=it+1
...
while(it<=4)
'set t 'it
'define dh2=(eh.3-h0)/h0(t=1)'
'define dr2=(er.3-r0)/r0(t=1)'
'define drr2=(err.3-rr0)/rr0(t=1)'
'd dh2'
'd dr2'
'd drr2'
it=it+1
...
* Plot the ENS8_prep_Heff_F*_rev.dat file containing (Harvest(date)-Harvest(date1))/Harvest(date1) for each scenario
*****************************************************************************************************************************
function main(args)
  rc = gsfallow("on")
  if (args='')
...
**************
dir=mydir
'open 'dir'/ENS8_CH_DATE_rev3.ctl'
...
'printim PRINT_tm_ENS_8mod_REFACUTE_2025/pr_CH_DATE_allF_'model'.png x1000 y800 white'
*******************************8
*************************************************
