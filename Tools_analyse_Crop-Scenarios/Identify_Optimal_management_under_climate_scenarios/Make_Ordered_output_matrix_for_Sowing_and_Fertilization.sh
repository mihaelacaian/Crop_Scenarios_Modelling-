#!/bin/sh
######  Ordonate direct model output parameters for calendaristic
######      sowing dates (after the dummy chosen perturbed sowing dates along mananement scenarios)
######  Redo the ordering per fertilization level

cat << eof1 > ordonate_per_spwind_date_and_fertilizaton.F90
! called in more Loops
! here for:  more Base-planting dates (npl); 1 Genotype, 1 model, 1 sceario
!              
...
! ordmin= value of the days ordered increasing, for days present in the TABLE above
! ordloc=index of increasing value location ordering of each day present in the TABLE above
!
                unit0=30
                unitt0=50
